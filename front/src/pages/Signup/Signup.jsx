...
function Signup() {
  ...
  const addUser = async (e) => {
    ...
    try {
      await signup({ name, firstname, email, password });
      setSuccessMessage(
        'Votre compte Atelier Héritage a été créé. Redirection vers la connexion...',
      );
        ...
        >
          <div className="signup-card-frame">
            ...
            <div className="mt-6 grid grid-cols-2 border-b border-[#b58a55]/25 text-center text-[10px] uppercase tracking-[0.12em] min-[380px]:text-[11px] sm:mt-8 sm:text-xs sm:tracking-[0.14em]">
              <Link to="/" className="pb-3 text-[#5f4a35] transition hover:text-[#17100b]">
                Connexion
              </Link>
              <span className="border-b-2 border-[#8b5a24] pb-3 font-semibold">
                Créer un compte
              </span>
            </div>
            ...
            <form onSubmit={addUser} className="mt-5 space-y-4 sm:mt-7" noValidate>
              {message && (
                <p role="alert" className="signup-alert signup-alert-error">
                  {message}
                </p>
              )}
              {successMessage && (
                <p role="status" className="signup-alert signup-alert-success">
                  {successMessage}
                </p>
              )}
...
function SignupField({
  ...
  return (
    <div>
      ...
      {help && (
        <p id={`${id}-help`} className="signup-field-help">
          {help}
        </p>
      )}
      {error && (
        <p id={`${id}-error`} className="signup-field-error">
          {error}
        </p>
      )}
    </div>
