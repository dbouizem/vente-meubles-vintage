  ...
  return (
    <div className="login-page relative min-h-dvh overflow-x-hidden bg-[#0b0907] font-ptMono text-[#ead7b8]">
      ...
      <header className="relative z-20 grid gap-3 px-3 py-4 text-[10px] uppercase tracking-[0.16em] min-[380px]:px-4 sm:grid-cols-3 sm:items-center sm:px-10 sm:py-6 sm:tracking-[0.18em]">
        <Link
          to="/accueil"
          className="inline-flex min-h-11 items-center gap-2 text-[#ead7b8] transition hover:text-[#c6a16b] focus-visible:outline-[#c6a16b] sm:gap-3"
        >
          <ArrowLeft size={16} />
          Retour à la boutique
        </Link>
        <div className="text-center sm:block">
          <p className="font-serif text-2xl uppercase tracking-[0.12em] text-[#f1dfbf] sm:text-3xl">
            Atelier Héritage
          </p>
          <p className="mt-1 text-[10px] tracking-[0.45em] text-[#c6a16b]">Paris</p>
        </div>
        <a
          href="mailto:contact@vintage.local"
          className="hidden items-center justify-end gap-3 text-[#ead7b8] underline underline-offset-4 transition hover:text-[#c6a16b] sm:inline-flex"
        >
          Besoin d’aide ?
          <HelpCircle size={18} />
        </a>
      ...
      <main className="relative z-10 grid gap-6 px-3 pb-7 min-[380px]:px-4 sm:px-8 sm:pb-10 lg:min-h-[calc(100dvh-96px)] lg:grid-cols-[minmax(280px,0.82fr)_minmax(620px,0.72fr)] lg:items-center lg:gap-12 lg:px-10 xl:grid-cols-[minmax(420px,1fr)_minmax(680px,820px)] xl:px-14">
        ...
        >
          <div className="max-w-xs">
            ...
            <p className="mt-8 text-xs uppercase tracking-[0.22em] text-[#ead7b8]/80">
              Atelier Héritage
            </p>
        ...
        >
          <div className="login-card-frame">
            <div className="text-center">
              ...
              <h1
                id="login-title"
                className="mt-3 font-serif text-4xl uppercase tracking-[0.08em] text-[#17100b] min-[380px]:mt-4 sm:text-5xl xl:text-6xl"
              >
                Bienvenue
              </h1>
              <p className="mt-2 font-aurore text-2xl leading-none text-[#3b2412] sm:mt-3 sm:text-4xl">
                chez Atelier Héritage
              </p>
            ...
            <div className="mt-6 grid grid-cols-2 border-b border-[#b58a55]/25 text-center text-[10px] uppercase tracking-[0.12em] min-[380px]:text-[11px] sm:mt-8 sm:text-xs sm:tracking-[0.14em]">
              ...
              <Link to="/signup" className="pb-3 text-[#5f4a35] transition hover:text-[#17100b]">
                Créer un compte
              </Link>
            </div>
            ...
            <form onSubmit={loginUser} className="mt-5 space-y-4 sm:mt-7 sm:space-y-5">
              {message && (
                <p
                  role="alert"
                  className="rounded-sm border border-red-900/30 bg-red-950/10 px-4 py-3 text-left text-sm text-red-900"
                >
              ...
              <div>
                <label
                  className="mb-2 block text-[11px] font-semibold uppercase tracking-[0.12em] sm:text-xs"
                  htmlFor="login-email"
                >
                  E-mail
                </label>
            ...
            <blockquote className="mt-5 text-center font-serif text-lg italic leading-relaxed text-[#2d1c10] sm:mt-6 sm:text-xl">
              ...
              <span className="mt-2 block font-aurore text-2xl text-[#5f3a1d] sm:mt-3 sm:text-3xl">
                Atelier Héritage
              </span>
            </blockquote>
          </div>
      ...
      <div className="relative z-10 px-4 pb-7 text-center text-[11px] uppercase tracking-[0.12em] text-[#ead7b8] sm:px-5 sm:pb-8 sm:text-xs sm:tracking-[0.16em] lg:hidden">
        Nouveau ici ?{' '}
        <Link to="/signup" className="underline underline-offset-4 transition hover:text-[#c6a16b]">
          Créez votre compte
        </Link>
      </div>
    </div>
