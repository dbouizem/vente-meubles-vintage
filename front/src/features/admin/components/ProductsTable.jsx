  ...
  return (
    <section className="mx-auto flex w-full max-w-6xl flex-col items-stretch min-h-[75vh] mt-3 px-3 py-8">
      <div className="mb-6 flex flex-col gap-4 text-left sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-gray-600">
            Créez, modifiez ou retirez les meubles affichés dans la boutique.
          </p>
        </div>
        <Link
          to="/create"
          className="inline-flex items-center justify-center rounded-md bg-dark-brown px-4 py-2 font-semibold text-white"
        >
          Ajouter un produit
        </Link>
      </div>
      {message && (
        <p
          role="status"
          className="mb-4 rounded-md border border-beige bg-white px-4 py-3 text-left text-sm text-dark-brown shadow-sm"
        >
          {message}
        </p>
      )}
              ...
              data.map((el) => (
                <tr key={el.id} className="border-t border-gray-100">
                  ...
                  <td className="flex gap-2 px-4 py-3">
                    ...
                    >
                      {' '}
                      <DeleteForeverIcon className="text-red-600" sx={{ fontSize: 36 }} />
                    </button>
                    <button
                      aria-label={`Modifier ${el.titre}`}
                      onClick={() => {
                        navigate(`/modif/${el.id}`);
                      }}
                    >
                      {' '}
      ...
      <Link
        to="/create"
        className="mt-5 self-center text-dark-brown sm:hidden"
